...
function onBeatHit()
    if curBeat == 168 then 
    doTweenAngle("transitionInknucks", "camGame", -360, 0.29, "quartOut") 
    if getPropertyFromClass('ClientPrefs', 'flashing') == true then   --checks if flashing lights are disabled, if they are, the camera flash doesnt happen
        cameraFlash("camHUD", "FF1111", 0.2)
     end
     
    end
    if curBeat == 136 then       
        doTweenAngle("transitionIn", "camGame", 360, 0.29, "quartOut")
        doTweenZoom("transitionInZoom", "camGame", getProperty("defaultCamZoom")+0.3, 0.29, "quartOut")    
        if getPropertyFromClass('ClientPrefs', 'flashing') == true then  --checks if flashing lights are disabled, if they are, the camera flash doesnt happen
            cameraFlash("camHUD", "0077FF", 0.2)
        end
    ...
    elseif (curBeat > 136 and curBeat < 168) or (curBeat > 169 and curBeat < 200)  then
        ...
        doTweenAngle('tt', 'camGame', angleshit, stepCrochet*0.002, 'circOut')  --Please
        xOffset = angleshit*16
    

    elseif curBeat == 200 then
        doTweenAngle("transitionOut", "camGame", -360, 0.3, "circInOut")
        doTweenZoom("transitionOutZoom", "camGame", getProperty("defaultCamZoom")-0.3, 0.29, "quartOut")  
